...
class Profile(models.Model):
    ...
    fecebook = models.CharField('facebook', max_length=100, null=True, blank=True)
    instagram = models.CharField('instagram', max_length=100, null=True, blank=True)
    #次、岸本さんが本番環境のアプリケーションのマイグレーションをする前にコメントアウト外してして試す？
    #The 'topimage' attribute has no file associated with it.
    #topimage = models.ImageField(upload_to='images', verbose_name='トップ画像', null=True, blank=True)
    #subimage = models.ImageField(upload_to='images', verbose_name='サブ画像', null=True, blank=True)
